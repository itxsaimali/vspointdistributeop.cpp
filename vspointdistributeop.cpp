...
//---------------------------------------------------------------------
{
    ...
    while(totEle!=0)
    {
        ...
        if (m_cic) {
                    ...
                    unsigned long long int totRows = toRow - fromRow + 1;
                    unsigned long long int baseRowsPerRank = totRows / size;
                    unsigned long long int remainder = totRows % size;
                    unsigned long long int local_nOfRow = baseRowsPerRank + (rank < remainder ? 1 : 0);

                    // Allocate memory for local data
                    float** local_fArray = new float*[m_nOfCol];
                    for (int i = 0; i < m_nOfCol; i++) {
                        local_fArray[i] = new float[local_nOfRow]();
                    }

                    // Scatter the data properly
                    float* sendBuffer = nullptr;
                    if (rank == 0) {
                        sendBuffer = new float[totRows * m_nOfCol];
                        for (unsigned long long int j = 0; j < totRows; j++) {
                            for (int i = 0; i < m_nOfCol; i++) {
                                sendBuffer[j * m_nOfCol + i] = m_fArray[i][j]; // Row-major order
                            }
                        }
                    }

                    unsigned long long int scatterCount = local_nOfRow * m_nOfCol;
                    float* recvBuffer = new float[scatterCount];
                    MPI_Scatter(sendBuffer, baseRowsPerRank * m_nOfCol, MPI_FLOAT,
                                recvBuffer, scatterCount, MPI_FLOAT, 0, MPI_COMM_WORLD);

                    //for (int i = 0; i < scatterCount; i++)
                    //    printf("PE = %d, recvBuffer[%d] = %f\n", rank, i, recvBuffer[i]);

                    if (rank == 0) {
                        delete[] sendBuffer;
                    }

                    // Convert recvBuffer to local_fArray
                    for (unsigned long long int j = 0; j < local_nOfRow; j++) {
                        for (int i = 0; i < m_nOfCol; i++) {
                            local_fArray[i][j] = recvBuffer[j * m_nOfCol + i];
                        }
                    }
                    delete[] recvBuffer;

                    // Allocate memory for local grid
                    float** local_grid = new float*[nOfField];
                    for (int i = 0; i < nOfField; i++) {
                        local_grid[i] = new float[m_numNewPts]();
                    }

                    for (int ptId = 0; ptId < local_nOfRow; ptId++) {
                        float px[3] = { local_fArray[0][ptId], local_fArray[1][ptId], local_fArray[2][ptId] };
                        float pos1 = (px[0] - m_origin[0]) / m_spacing[0];
                        float pos2 = (px[1] - m_origin[1]) / m_spacing[1];
                        float pos3 = (px[2] - m_origin[2]) / m_spacing[2];

                        int i1 = floor(pos1), i2 = floor(pos2), i3 = floor(pos3);
                        int i11 = i1 + 1, i21 = i2 + 1, i31 = i3 + 1;

                        float dd1 = pos1 - i1, dd2 = pos2 - i2, dd3 = pos3 - i3;
                        float de1 = 1.0 - dd1, de2 = 1.0 - dd2, de3 = 1.0 - dd3;

                        float d[8] = { de1 * de2 * de3, dd1 * de2 * de3, de1 * dd2 * de3, dd1 * dd2 * de3,
                                    de1 * de2 * dd3, dd1 * de2 * dd3, de1 * dd2 * dd3, dd1 * dd2 * dd3 };

                        unsigned long long int jkFactor = m_sampleDimensions[1] * m_sampleDimensions[0];
                        unsigned long long int jFactor = m_sampleDimensions[0];
                        unsigned long long int ind[8] = { i3 * jkFactor + i2 * jFactor + i1,
                        ...
                        for (int n = 0; n < 8; n++) {
                            if (ind[n] < nCell) {
                                for (int j = 0; j < fieldList.size(); j++) {
                                    float norm = m_useConstant ? m_constValue : local_fArray[3 + j][ptId];
                                    local_grid[j][ind[n]] += d[n] * norm / cellVolume;
                                    wc += d[n] * norm;
                                }
                            }
                        }
                        ...
                        for (int i = 0; i < nOfField; i++) {
                            float sum_local = 0.0;
                            for (int j = 0; j < m_numNewPts; j++) {
                                sum_local += local_grid[i][j];
                            }
                            std::cout << "Rank " << rank << ", Field " << i << ", Local sum: " << sum_local << std::endl;
                        }*/

                    // Gather results
                    float** global_grid = nullptr;
                    if (rank == 0) {
                    ...
                    for (int i = 0; i < nOfField; i++) {
                        MPI_Reduce(local_grid[i], (rank == 0) ? global_grid[i] : nullptr, m_numNewPts, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
                    ...
                    // Cleanup
                    for (int i = 0; i < m_nOfCol; i++) delete[] local_fArray[i];
                    delete[] local_fArray;
                    for (int i = 0; i < nOfField; i++) delete[] local_grid[i];
                    delete[] local_grid;
                    if (rank == 0) {
                        for (int i = 0; i < nOfField; i++) delete[] global_grid[i];
                        delete[] global_grid;
                    }
